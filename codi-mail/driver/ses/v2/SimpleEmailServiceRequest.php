...
{
	private $ses, $verb, $parameters = array(), $path = '/', $jsonPayload = null;
	...
	* @return SimpleEmailServiceRequest $this
	*/
	public function setJsonPayload($payload) {
	...
	*/
	public function getResponse() {
        ...
        $is_json_payload = !is_null($this->jsonPayload);
        $is_v2 = strpos($this->path, '/v2/') === 0;
        $url = 'https://'.$this->ses->getHost().$this->path;
        $query = implode('&', $this->getParametersEncoded());
        $payload = $is_json_payload ? json_encode($this->jsonPayload) : '';
        $string_to_sign_payload = $is_json_payload ? $payload : ($this->verb === 'POST' ? $query : '');
        $headers = $this->getHeaders($string_to_sign_payload, $query);
		...
		if ($this->response->error === false && !empty($this->response->body)) {
			if ($is_v2) {
				...
				if (!in_array($this->response->code, array(200, 201, 202, 204))) {
					...
					$output['curl'] = false;
					$output['Error'] = array();
					$output['Error']['Type'] = isset($error->__type) ? (string)$error->__type : '';
					$output['Error']['Code'] = isset($error->__type) ? (string)$error->__type : (string)$this->response->code;
					$output['Error']['Message'] = isset($error->message) ? (string)$error->message : (isset($error->Message) ? (string)$error->Message : '');
					$output['RequestId'] = '';
		...
		} else if ($this->response->error === false && !in_array($this->response->code, array(200, 201, 202, 204))) {
			$this->response->error = array(
				'curl' => false,
				'Error' => array(
					'Type' => '',
					'Code' => (string)$this->response->code,
					'Message' => '',
				),
				'RequestId' => '',
			);
     ...
     * @return array
     */
	protected function getHeaders($payload, $query = '') {
        $headers = array();
	    ...
	    if ($this->ses->getRequestSignatureVersion() == SimpleEmailService::REQUEST_SIGNATURE_V4) {
            ...
            $headers[] = 'Authorization: ' . $this->__getAuthHeaderV4($date, $payload, $query);
	...
	*/
     ...
     * @return string
     */
    private function __getAuthHeaderV4($amz_datetime, $payload, $query = '') {
        $amz_date = substr($amz_datetime, 0, 8);
        $algo = 'sha256';
        $aws_algo = 'AWS4-HMAC-' . strtoupper($algo);

        $host_parts = explode('.', $this->ses->getHost());
        $service = strpos($this->path, '/v2/') === 0 ? 'ses' : $host_parts[0];
        $region = $host_parts[1];

        $canonical_uri = $this->path;
        $canonical_querystring = $this->verb === 'POST' ? '' : $query;
